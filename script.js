...
function fetchMovieData(searchQuery = '') {
    ...
    fetch(url)
        .then(response => {
            if (!response.ok) {
                throw new Error('Couldn\'t fetch the movie');
            }
        ...
        .catch(error => {
            console.log('Couldn\'t fetch the movie:', error);
        });
}
...
function displayMovieList(movies) {
    ...
    movies.forEach(movie => {
        ...
        movieItem.innerHTML = `
            ...
            <img src="https://image.tmdb.org/t/p/w200${movie.poster_path}"alt="${movie.title}" class=>
        `;
